...
interface ImageDao {
    ...
    @Query("SELECT * FROM image WHERE status = :status ORDER BY postEntityId DESC, id ASC LIMIT 1")
    suspend fun getNextPending(status: Status = Status.PENDING): Image?

    @Query("SELECT * FROM image WHERE status = :status AND postEntityId NOT IN (:excludePostIds) ORDER BY postEntityId DESC, id ASC LIMIT 1")
    suspend fun getNextPendingExcludePosts(excludePostIds: List<Long>, status: Status = Status.PENDING): Image?

    @Query("SELECT * FROM image WHERE status = :status AND postEntityId IN (:postIds) ORDER BY postEntityId DESC, id ASC LIMIT 1")
